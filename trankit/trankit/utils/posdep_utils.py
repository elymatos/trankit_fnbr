...
def _fnbr_type_from_misc(misc):
    for item in str(misc).split('|'):
        if item.startswith('FNBRType='):
            value = item.split('=', 1)[1]
            return value if value in FNBR_TYPES else '_'
    return '_'
        ...
        for token in sentence:
            if len(token[ID]) == 2:
                ...
                if get_vocab:
                    ...
                    fnbr_type = _fnbr_type_from_misc(token.get(MISC, '_'))
                ...
                new_ex[HEAD].append(head)
                new_ex[DEPREL].append(deprel)
                new_ex[FNBR_TYPE].append(fnbr_type)

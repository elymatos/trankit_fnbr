...
instance_fields = [
    ...
    'head_idxs', 'deprel_idxs', 'lemma_type_idxs', 'word_mask'
]
...
class TaggerDatasetLive(Dataset):
    ...
    def load_data(self):
        ...
        for inst in self.data:
            ...
            if len(flat_pieces) > self.max_input_length - 2:
                ...
                for key in ['words', 'word_ids', LEMMA, UPOS, XPOS, FEATS, HEAD, DEPREL, FNBR_TYPE, 'flat_pieces']:
                    cur_inst[key] = []

                for i in range(len(inst['words'])):
                    for key in ['words', 'word_ids', LEMMA, UPOS, XPOS, FEATS, HEAD, DEPREL, FNBR_TYPE]:
    ...
    def numberize(self):
        ...
        for inst in self.data:
            ...
            lemma_type_idxs = [self.vocabs.get(FNBR_TYPE, {}).get(label, -100) for label in inst[FNBR_TYPE]]
            ...
            instance = Instance(
                ...
                lemma_type_idxs=lemma_type_idxs,
                word_mask=word_mask
            )
    ...
    def collate_fn(self, batch):
        ...
        batch_head_ids, batch_deprel_ids, batch_lemma_type_ids, batch_word_mask = [], [], [], []
        ...
        for inst in batch:
            ...
            batch_lemma_type_ids.extend(inst.lemma_type_idxs + [-100] * (max_word_num - inst.word_num))
        ...
        batch_lemma_type_ids = torch.tensor(batch_lemma_type_ids, dtype=torch.long, device=self.config.device)
        ...
        return Batch(
            ...
            lemma_type_idxs=batch_lemma_type_ids,

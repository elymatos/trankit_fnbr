...
class PosDepClassifier(nn.Module):
    def __init__(self, config, treebank_name):
        ...
        # FNBr semantic typing is independent from UPOS/XPOS.
        self.lemma_type_stoi = self.vocabs.get(FNBR_TYPE)
        self.lemma_type_itos = ({index: label for label, index in self.lemma_type_stoi.items()}
                                if self.lemma_type_stoi else {})
        self.lemma_type_ffn = (nn.Linear(self.xlmr_dim, len(self.lemma_type_stoi))
                               if self.lemma_type_stoi else None)
    ...
    def forward(self, batch, word_reprs, cls_reprs):
        ...
        lemma_type_loss = None
        if self.lemma_type_ffn is not None and torch.any(batch.lemma_type_idxs != -100):
            lemma_type_scores = self.lemma_type_ffn(word_reprs).view(-1, len(self.lemma_type_stoi))
            lemma_type_loss = self.criteria(lemma_type_scores, batch.lemma_type_idxs)
            loss += lemma_type_loss
        if getattr(self.config, 'lemma_type_only', False):
            if lemma_type_loss is None:
                raise ValueError('lemma_type_only training requires FNBRType labels')
            return lemma_type_loss
    ...
    def predict(self, batch, word_reprs, cls_reprs):
        ...
        lemma_type_predictions = None
        if self.lemma_type_ffn is not None:
            probabilities = F.softmax(self.lemma_type_ffn(word_reprs), dim=2)
            # Return the complete ranking; API consumers choose top-k without
            # rerunning the model.
            top_k = len(self.lemma_type_stoi)
            values, indices = torch.topk(probabilities, top_k, dim=2)
            lemma_type_predictions = []
            for batch_index in range(probabilities.size(0)):
                sentence = []
                for word_index in range(batch.word_num[batch_index]):
                    sentence.append([
                        {
                            'type': self.lemma_type_itos[int(label_index)],
                            'probability': float(probability)
                        }
                        for probability, label_index in zip(
                            values[batch_index, word_index].detach().cpu(),
                            indices[batch_index, word_index].detach().cpu()
                        )
                    ])
                lemma_type_predictions.append(sentence)
        return predicted_upos, predicted_xpos, predicted_feats, dep_preds, lemma_type_predictions
